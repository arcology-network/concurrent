...
func TestNativeStorage(t *testing.T) {
	...
	_, err, _, _ := exectest.DeployThenInvoke(targetPath, "native/NativeStorage.sol", "0.8.19", "NativeStorage", "call()", []byte{}, false)
	if err != nil {
		t.Error(err)
	}
}
...
func TestGasDebitInFailedTx(t *testing.T) {
	currentPath, _ := os.Getwd()
	targetPath := path.Dir(filepath.Dir(currentPath))
	_, err, _, _ := exectest.DeployThenInvoke(targetPath, "native/NativeStorage.sol", "0.8.19", "TestFailed", "call()", []byte{}, false)
	if err != nil {
		t.Error(err)
	}
}
